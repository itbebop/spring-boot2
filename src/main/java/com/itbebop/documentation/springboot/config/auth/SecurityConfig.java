...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception{
        http
                .csrf().disable().headers().frameOptions().disable() // h2-console 화면을 사용하기 위해 disable함
                .and().authorizeRequests() // URL별 권한관리를 설정하는 옵션의 시작점임. 이게 선언되어야 antMatchers 옵션 사용가능
                ...
                /*
                 ㅇ antMatchers
                  ...
                  - "/" 등 지정된 URL들은 permitAll() 옵션을 통해 전체 열람 권한을 준 것
                  - "/api/v1/**" 주소를 가진 API는 USER 권한을 가진 사람만 가능하도록 함
                 ㅇ anyRequest
                  - 설정된 값들 이외 나머지 URL들을 나타냄
                  - 여기서는 authenticated()을 추가하여 나머지 URL들은 모두 인증된(로그인 한) 사용자들에게만 허용하게 함
                 ㅇ logout().logoutSuccessUrl("/")
